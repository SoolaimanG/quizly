...
export const Tabs: React.FC<TabsProps> = ({ header, elements }) => {
  const [active, setActive] = useState(header[0]);

  return (
    <div className="w-full">
      <div className="w-full gap-3 flex items-center justify-between border-b-[1.5px] border-gray-300 border-solid">
        {header.map((h, i) => (
          <button
            //layoutId={active === h ? "tab-button" : ""}
            onClick={() => setActive(h)}
            className={cn(
